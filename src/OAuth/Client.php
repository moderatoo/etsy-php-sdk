...
class Client {
    ...
    public function __call($method, $args) {
        ...
        $uri = $args[0];
        if($method == 'get' && count(isset($args[1])?$args[1]:[])) {
            $uri .= "?".RequestUtil::prepareParameters($args[1]);
        }
        if(in_array($method, ['post', 'put'])) {
            if($file = RequestUtil::prepareFile(isset($args[1])?$args[1]:[])) {
                $opts['multipart'] = $file;
            }
            else {
                $opts['form_params'] = isset($args[1])?$args[1]:[];
            }
        }
        if($method == 'DELETE' && count(isset($args[1])?$args[1]:[])) {
            $opts['query'] = $args[1];
        }
        ...
        try {
            ...
            if($status_code == 404 && !(isset($this->config['404_error'])?$this->config['404_error'] : false)) {
     ...
     * @return string
     */
    public function createNonce($bytes = 12) {
